...
i_net_loaded = DehazeFormer().to(device)

# Load the saved state dictionary
i_net_loaded.load_state_dict(torch.load(model_path, map_location=device))

# Set the model to evaluation mode if testing inference
i_net_loaded.eval()

# Run a forward pass with dummy input to check
dummy_input = torch.randn(1, 3, 128, 128).to(device)
output = i_net_loaded(dummy_input)
